...
main_window.title("Zé Gargalhadas")
...
def roda_Chatbot():      
    ...
    if entrada_nome_usuario:
        ...
        if entrada_sugestao:
            ...
            entrada_sugestao = False
            historico_conversa+="\n Agoora eu compreendi! Vamos continuar nosso pequeno show? \n"
            v.set(historico_conversa)
            
        else:
            resposta = pc.buscaResposta_GUI("Cliente: "+texto+"\n")
            
            if resposta == "Se me desculpe, mas não sei o que falar sobre isso":
                historico_conversa += "\n Se me desculpe, não sei o que falar. O que você esperava? \n"
                v.set(historico_conversa)
